...
//	 URL:http://unixweb.kutztown.edu/~tturn265/Round3.php
//	 About: Displays the round 3 pairings to the use-->
	...
	<body>
		<div id="container" style="width:100%;height:100%;">
			<div id="header" style="Text-align:center;border-style:solid;border-color:black">
				<h1 style="margin-bottom:0;">Round 3</h1>
			</div>
			...
			$Round3=$_SESSION['Standings'];
			
			$_SESSION['Round3']=$Round3;
			//converts player Id to player name 
			for ($x =$numPlay; $x>=0 ; $x--){
			$Round3[$x]=$Players[$Round3[$x]];
			}
	
			$_SESSION['Round3']=$Round3;
			//checks to see if a player gets a bye
		if($numPlay%2==1)
			{
			$Name=$Round3[$numPlay];
			...
			//display text boxes based on the number of players 
			echo("<div id='content' style='height:50%;width:75%;float:right;'>");
			echo("<form onsubmit='return check();' action= 'Standings3.php' method='post'>");
			if($numPlay>=2){
			echo($Round3[1]);
			...
			echo("<input type= 'submit' name='submit' value='Finish round 3' />");
			echo("</form>");
			echo("</div>");
			?>
			</div>
